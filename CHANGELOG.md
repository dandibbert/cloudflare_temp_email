...
- feat: |邮件拒收| 管理后台新增邮件标题和正文关键词黑名单，对解码后的标题、纯文本与 HTML 可见文字做不区分大小写匹配，命中后在落库及所有外部动作前拒收

### Bug Fixes

- fix: |邮件拒收| 发件人规则改为去空项、统一大小写并按精确邮箱/域名边界/普通关键词匹配；清空后台列表会同步删除 KV 旧值
- fix: |邮件拒收| 垃圾邮件检查仅信任配置的 `Authentication-Results` authserv-id，默认 `mx.cloudflare.net`，避免原始邮件伪造认证结果绕过拒收
- fix: |邮件拒收| 未知地址检查提前到 MIME 读取前，规则读取、地址查询、认证/内容解析及存储失败时停止处理，避免检查异常后继续接收或触发转发、Webhook、Telegram 和自动回复
...
- test: |邮件拒收| 新增发件人边界、空规则、可信认证头、标题/纯文本/HTML 关键词、清空 KV 及未知地址拒收回归测试
